...
const monthArray=Array(days).fill(null).map((_,i)=>i+1);
...
// }
  ...
  <div className="grid grid-cols-7 gap-2 p-4">
       {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d,index) => (
        <div key={index} className="text-center font-semibold">{d}</div>
       ))}
...
{monthDays.map((day, idx) => (
  ...
  >
    ...
    {day && events
      ...
      .map(e => (
        <div key={e.id} className="text-xs bg-blue-100 p-1 rounded mt-1">
          {e.title}
        </div>
      ))}
  </div>
